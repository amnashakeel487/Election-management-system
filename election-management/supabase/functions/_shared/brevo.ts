...
export function getBrevoSender(fallbackFrom?: string): BrevoSender {
  ...
  const fromEnv =
    fallbackFrom?.trim() ||
    Deno.env.get('SECRET_VOTER_ID_FROM_EMAIL')?.trim() ||
    Deno.env.get('CREATOR_APPROVAL_FROM_EMAIL')?.trim()
  if (fromEnv) {
    return parseSender(fromEnv, name)
  }
  throw new Error('Set BREVO_SENDER_EMAIL (and optional BREVO_SENDER_NAME) in Edge Function secrets')
}
