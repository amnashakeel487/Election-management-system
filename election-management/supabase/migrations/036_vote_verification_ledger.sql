-- Vote verification ledger: hashed secret voter IDs per candidate (no names)

alter table public.anonymous_ballots
  add column if not exists voter_proof_hash text;

create index if not exists anonymous_ballots_election_proof_hash_idx
...
begin
  ...
  where ab.election_id = p_election_id
    and ab.voter_proof_hash is null;
  ...
  select coalesce(
    jsonb_agg(
      jsonb_build_object(
        ...
        'proof_hashes', coalesce(vc.hashes, '[]'::jsonb)
      )
  ...
  left join (
    select
      ...
      jsonb_agg(ab.voter_proof_hash order by ab.cast_at asc) filter (
        where ab.voter_proof_hash is not null
      ) as hashes
    from public.anonymous_ballots ab
    where ab.election_id = p_election_id
  ...
  return jsonb_build_object(
    ...
    'candidates', v_rows,
    'legacy_ballots_without_hash', v_legacy,
    'hash_algorithm', 'sha256(upper(trim(secret_id)) || '':"" || election_id)'
  );
end;
$$;
...
-- Store proof hash when casting (extends 035 cast)
...
begin
  ...
  insert into public.anonymous_ballots (election_id, candidate_id, voter_proof_hash)
  values (p_election_id, p_candidate_id, v_proof_hash)
